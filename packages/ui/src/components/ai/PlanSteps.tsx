...
const MARK: Record<PlanStepStatus, { glyph: string; color: string }> = {
  pending: { glyph: '○', color: 'var(--cds-text-tertiary)' },
  active:  { glyph: '◐', color: 'var(--cds-action-primary)' },
  done:    { glyph: '●', color: 'var(--cds-feedback-success)' },
  error:   { glyph: '✕', color: 'var(--cds-feedback-error)' },
};
 ...
 * Spec: specs/plan-steps/index.md
 */
    ...
    {steps.map((s, i) => {
      ...
      return (
        <Group key={i} gap={8} align="center">
          ...
          <Text size="sm" style={{ color: s.status === 'pending' ? 'var(--cds-text-tertiary)' : 'var(--cds-text-primary)' }}>
            {s.label}
          </Text>
        </Group>
      );
