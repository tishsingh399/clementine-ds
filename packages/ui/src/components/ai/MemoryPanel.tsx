 ...
 * deletable (user control over memory). Tokens: memory.* . Spec: specs/ai/memory-panel/index.md
 */
export const MemoryPanel = forwardRef<HTMLDivElement, MemoryPanelProps>(
  ({ facts, title = 'Memory', onDelete }, ref) => (
    <Paper ref={ref} withBorder radius="md" p="sm" style={{ background: 'var(--tds-surface-elevated)' }}>
      <Text size="xs" fw={600} mb={8} style={{ textTransform: 'uppercase', color: 'var(--tds-text-tertiary)' }}>{title}</Text>
      <Stack gap={6}>
        {facts.map((f) => (
          <Group key={f.id} justify="space-between" wrap="nowrap" gap="sm">
            <Text size="sm" style={{ color: 'var(--tds-text-primary)' }}>{f.text}</Text>
