...
const LINE: Record<DiffLine['type'], { bg: string; prefix: string }> = {
  add:     { bg: 'var(--tds-feedback-success-subtle)', prefix: '+' },
  remove:  { bg: 'var(--tds-feedback-error-subtle)',   prefix: '-' },
  context: { bg: 'transparent',                        prefix: ' ' },
};

/**
 * Tray 4 · AI — a proposed change as added/removed lines with accept/reject. Pairs with
 * an agent suggesting an edit. Tokens: diff.* . Spec: specs/ai/diff-view/index.md
 */
export const DiffView = forwardRef<HTMLDivElement, DiffViewProps>(({ lines, onAccept, onReject }, ref) => (
  <Paper ref={ref} withBorder radius="md" style={{ overflow: 'hidden', background: 'var(--tds-surface-elevated)' }}>
    <div style={{ fontFamily: 'var(--mantine-font-family-monospace)', fontSize: 12 }}>
      {lines.map((l, i) => (
        <div key={i} style={{ background: LINE[l.type].bg, padding: '2px 12px', whiteSpace: 'pre', color: 'var(--tds-text-primary)' }}>
          {LINE[l.type].prefix} {l.text}
        </div>
      ))}
    </div>
    {(onAccept || onReject) && (
      <Group gap="xs" p="xs" style={{ borderTop: '1px solid var(--tds-border-default)' }}>
