 ...
 * it fills. Tokens: context-meter.* . Spec: specs/context-meter/index.md
 */
export const ContextMeter = forwardRef<HTMLDivElement, ContextMeterProps>(({ used, total }, ref) => {
  const pct = Math.min(used / total, 1);
  const warn = pct > 0.85;
  const fill = warn ? 'var(--cds-feedback-warning)' : 'var(--cds-action-primary)';
  return (
    <div ref={ref} style={{ width: 220 }} role="meter" aria-valuenow={used} aria-valuemin={0} aria-valuemax={total} aria-label="Context window used">
      <Group justify="space-between" mb={4}>
        ...
        <Text size="xs" style={{ color: warn ? 'var(--cds-feedback-warning)' : 'var(--cds-text-tertiary)' }}>
          {used.toLocaleString()} / {total.toLocaleString()}
        </Text>
      </Group>
      <div style={{ height: 6, borderRadius: 99, background: 'var(--cds-surface-subtle)', overflow: 'hidden' }}>
