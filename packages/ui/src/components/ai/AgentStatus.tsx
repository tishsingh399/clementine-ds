...
const MAP: Record<AgentState, { color: string; text: string; busy?: boolean }> = {
  idle:     { color: 'var(--cds-text-tertiary)',    text: 'Idle' },
  thinking: { color: 'var(--cds-action-primary)',   text: 'Thinking…', busy: true },
  tool:     { color: 'var(--cds-action-primary)',   text: 'Calling tool…', busy: true },
  waiting:  { color: 'var(--cds-feedback-warning)', text: 'Waiting on you' },
  done:     { color: 'var(--cds-feedback-success)', text: 'Done' },
  error:    { color: 'var(--cds-feedback-error)',   text: 'Error' },
  refused:  { color: 'var(--cds-text-tertiary)',    text: 'Declined' },
};
 ...
 * Spec: specs/agent-status/index.md
 */
export const AgentStatus = forwardRef<HTMLDivElement, AgentStatusProps>(({ state, label }, ref) => {
  ...
  return (
    <Group ref={ref} gap={6} align="center" role="status" aria-live="polite">
      ...
      <Text size="sm" style={{ color: 'var(--cds-text-secondary)' }}>{label ?? m.text}</Text>
    </Group>
  );
});
